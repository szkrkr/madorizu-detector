...
def find_rooms(img, noise_removal_threshold=25, corners_threshold=0.1,room_closing_max_length=100, gap_in_wall_threshold=500):
    ...
    cv2.imwrite('./_3.jpg', mask)

    img = ~mask

    # Detect corners (you can play with the parameters here)
    dst = cv2.cornerHarris(img ,2,3,0.04)
    cv2.imwrite('./_4.jpg', dst)
    dst = cv2.dilate(dst,None)
    cv2.imwrite('./_5.jpg', dst)
...
#Read gray image
img = cv2.imread("./original/dymmy.jpg", cv2.IMREAD_GRAYSCALE )
rooms, colored_house = find_rooms(img.copy())
#print(rooms)
cv2.imwrite('./_res.jpg', colored_house)
cv2.waitKey()
